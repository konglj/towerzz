  ...
  $.danidemo = $.extend( {}, {
    ...
    addFile: function(id, i, file){
    	var name=file.name;
    		...
    		var sFileName=file.name.replace(/.*(\/|\\)/, "");
    		var fileExt=(/[.]/.exec(name)) ? /[^.]+$/.exec(name.toLowerCase()) : ''; 
    	if(sFileName.length>10){
    		sFileName=sFileName.substring(0,9);
    		name=sFileName+"...."+fileExt;
		...
		var template = '<div id="demo-file' + i + '">' +
		                   ...
		                   '<span class="demo-file-id"></span>' + name + ' <span class="demo-file-size">(' + $.danidemo.humanizeSize(file.size) + ')</span><br />状态: <span class="demo-file-status">等待上传</span>'+
	...
	updateFileProgress: function(i, percent){
		...
		$('#demo-file' + i).find('span.sr-only').html(percent + ' 已完成');
	},
